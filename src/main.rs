...
fn do_benchmark() {
	...
	let mut out: [u8; out_size] = [0; out_size];
		...
		post::scrypt_positions(provider.id, &id, 0, LABELS_COUNT as u64 - 1, LABEL_SIZE, &salt, 0, &mut out, 512, 1, 1, &mut hashes_computed as *mut u64, &mut hashes_per_sec as *mut u64);
		println!("{}: {} hashes, {} h/s", provider.model, hashes_computed, hashes_per_sec);
 	    }
	}
    }
}
/*
fn do_test(label_size: i32, labels_count: i32, reference_rrovider: i32, print_result: bool) {
}
*/
...
fn do_providers_list() {
    ...
    for provider in &providers {
	println!("{}: [{}] {}", provider.id, get_provider_class_string(provider.compute_api), provider.model);
    }
}
